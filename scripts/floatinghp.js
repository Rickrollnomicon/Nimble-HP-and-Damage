...
import { requestApplyHpDelta, requestToggleStatus, initHPDamageRelay, MODULE_ID } from "./relay.js";
...
async function _postTargetedChatCard({ entries = [], verbOverride = null, armorOverride = false, defendMeta = null }) {
  try {
...
html.find('[data-action="fury-keep"]').off("click").on("click", async (ev) => {
  ...
  async changeHPForTargets(delta = 0, targetPool = null, options = {}) {
    ...
    await _postTargetedChatCard({ entries, armorOverride: !!this._tgtArmorOverride, defendMeta });
...
Hooks.on("init", () => {
  ...
  Hooks.on("renderChatMessage", (message, html) => {
    try {
      ...
      // v13 message header sender element
